...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'verticals.despatxos' });
  ...
  return {
    ...
    alternates: {
      canonical: `${siteUrl}/${locale}/despatxos`,
      languages: Object.fromEntries(routing.locales.map(l => [l, `${siteUrl}/${l}/despatxos`])),
    },
    openGraph: {
      title: t('meta.title'),
      description: t('meta.description'),
      url: `${siteUrl}/${locale}/despatxos`,
...
export default async function Despatxos({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
