...
public class AmgAdminCommandService {
    ...
    public AmgAdminCommandService(SystemConfigService sysConfig,
    ...
    public void handle(Long chatId, String text) {
        ...
        if (cmd.equals("/ajuda") || cmd.equals("/help") || cmd.equals("/start")) {
            ...
            response = buildPipeline();
        } else if (cmd.equals("/nous") || cmd.equals("/new")) {
            response = buildNewLeads();
        } else if (cmd.equals("/leads")) {
            response = buildRecentLeads();
        } else if (cmd.equals("/pressupostos") || cmd.equals("/budgets")) {
            response = buildPendingBudgets();
        } else if (cmd.equals("/tenants") || cmd.equals("/clients")) {
            response = buildActiveTenants();
        } else if (cmd.equals("/stats")) {
            response = buildStats();
        } else {
            // Qualsevol text que no sigui una comanda → agent IA
            aiService.handleAsync(chatId, text);
            return;
        }
    ...
    // ── Mètodes de dades (públics, usats també per AmgAdminAiService via tools) ─
    ...
    public String buildRecentLeads() {
        UUID ownerTenantId = tenantRepository.findByIsOwnerTrue().map(Tenant::getId).orElse(null);
        List<Lead> leads = ownerTenantId != null
    ...
    public String buildPendingBudgets() {
    ...
    public String buildActiveTenants() {
    ...
    private String helpText() {
        return """
                ...
                💬 <i>O escriu qualsevol pregunta en llenguatge natural i l'agent IA et respondrà.</i>""";
    }
